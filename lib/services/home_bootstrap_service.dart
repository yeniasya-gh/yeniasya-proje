...
class HomeBootstrapService {
  ...
  Future<HomeBootstrapPayload> fetch() async {
    final uri = Uri.parse("$_baseUrl/home/bootstrap");
    final response = await _client.get(uri).timeout(timeout);

    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw Exception("HOME_BOOTSTRAP_HTTP_${response.statusCode}");
    }

    final json = jsonDecode(response.body);
    if (json is! Map) {
      throw Exception("HOME_BOOTSTRAP_INVALID_PAYLOAD");
    }
    final data = Map<String, dynamic>.from(json);
    if (data["ok"] == false) {
      throw Exception(
        data["error"]?.toString() ?? "HOME_BOOTSTRAP_FAILED",
      );
    }
